...
if C.isoform_dense_build_splice_db:
  cmds.append("STAR --runMode genomeGenerate --genomeDir %s --genomeFastaFiles %s --sjdbFileChrStartEnd %s %s" % (cor(C.__isoform_dense_genome_generate_dir__), split_fasta, cor(C.__isoform_dense_star_splice_output__), cor(C.star_gg_opts)));
else:
