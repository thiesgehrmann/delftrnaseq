...
M.add_step("CDS_BED", "${POST_STAR_AL_INDEX_OUT}", ' '.join(C.__cds_bed_output__()), 'pipeline_cds_bed.py');
...
if C.perform_quality_report:
    if C.check_contamination :
        M.add_step("QUALITYREPORT", "${TRIMMOMATIC_OUT} ${STAR_AL_OUT} ${CDS_BED_OUT} ${UNMAPPED_OUT}", ' '.join(flatten(C.__quality_output__())), 'pipeline_quality.py');
    else :
        M.add_step("QUALITYREPORT", "${TRIMMOMATIC_OUT} ${STAR_AL_OUT} ${CDS_BED_OUT}", ' '.join(flatten(C.__quality_output__())), 'pipeline_quality.py');
#fi
